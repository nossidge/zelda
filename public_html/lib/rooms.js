...
var ModRooms = (function () {
  ...
  // Get rooms by ID.
  // This is the ID starting from 1 that is generated by Ruby.
  // This corresponds to the DOT mission graph.
  // There may be missing IDs due to removal of dead-end rooms.
  // Ordered by keys for easier debugging.
  var hashByID = function(objJSON) {
...
//        logReason = (room.lock_group && room.lock_group.multi_type == 'crystals');
  ...
  var resolvePatterns = function(objJSON) {
    ...
    if (duplicateRoomIDs.length == 0) {
      ...
      for (var roomID in objJSON.patterns_possible) {
        if (objJSON.patterns_possible.hasOwnProperty(roomID)) {
          var room = objJSON.room_by_id[roomID];
    ...
    //   call, but we should end up with no clashes.
      ...
      for (var roomID in objJSON.patterns_possible) {
        if (objJSON.patterns_possible.hasOwnProperty(roomID)) {
          ...
          objJSON.room_by_id[roomID].patterns_possible = validPatterns;
        }
      }
